...
class OpenAIClient:
    ...
    # 发送消息
    def send_messages(self, messages_get, tools=None):
        """
...
def init_func_call():
    ...
    system_tools += arm_control.get_func_tools_list()
    system_tools += car_control.get_func_tools_list()
    system_tools += vision_sensor.get_func_tools_list()
    system_tools += audio_sensor.get_func_tools_list()
    system_tools += map_sensor.get_func_tools_list()
...
def main():
    ...
    client = OpenAIClient(
        ...
        base_url=base_url,
        model=model,
        system_prompt=system_prompt_tools_cn,
        tools=system_tools
    )
    ...
    message = client.send_messages(messages_get = [{"role": "user", "content": questions0}], tools = system_tools)
