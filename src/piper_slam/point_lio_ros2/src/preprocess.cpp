...
void Preprocess::unilidar_handler(const sensor_msgs::msg::PointCloud2::SharedPtr &msg)
{
    ...
    float intensity_avg = 0;
    float curvature_avg = 0;
    for (int i = 0; i < plsize; i++)
    {
      PointType added_pt;
      ...
      intensity_avg += added_pt.intensity;
      curvature_avg += added_pt.curvature;
    ...
    std::cout << intensity_avg/plsize << " " << curvature_avg/plsize << " " << plsize << std::endl;
